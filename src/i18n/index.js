...
export const translations = {
  'zh-CN': {
    ...
    globalGroupNodeSelect: 'GLOBAL 组默认走节点选择',
    globalGroupNodeSelectTip: '仅 Clash 生效。显式定义 GLOBAL 代理组，默认指向「节点选择」而非 DIRECT，使 Clash 切到 global 模式时立即走代理',
    skipTlsVerify: '跳过 TLS 证书验证',
    skipTlsVerifyTip: '仅 Node.js/Docker 部署生效。允许获取无有效 TLS 证书的订阅地址，Cloudflare Workers 不支持',
  ...
  'en-US': {
    ...
    globalGroupNodeSelect: 'GLOBAL group defaults to Node Select',
    globalGroupNodeSelectTip: 'Clash only. Defines an explicit GLOBAL proxy-group pointing to Node Select by default, instead of DIRECT, so Clash global mode routes through proxy immediately',
    skipTlsVerify: 'Skip TLS Verification',
    skipTlsVerifyTip: 'Node.js/Docker only. Allows fetching subscriptions without valid TLS certificates. Not supported on Cloudflare Workers',
  ...
  'fa': {
    ...
    globalGroupNodeSelect: 'گروه GLOBAL به طور پیش‌فرض از Node Select پیروی می‌کند',
    globalGroupNodeSelectTip: 'فقط Clash. یک گروه پروکسی GLOBAL صریح تعریف می‌کند که به طور پیش‌فرض به Node Select اشاره می‌کند، نه DIRECT، تا حالت global فوراً از پروکسی استفاده کند',
    skipTlsVerify: 'ردبندی TLS را رد کنید',
    skipTlsVerifyTip: 'فقط Node.js/Docker. امکان دریافت اشتراک بدون گواهی TLS معتبر. در Cloudflare Workers پشتیبانی نمی‌شود',
  ...
  'ru': {
    ...
    globalGroupNodeSelect: 'Группа GLOBAL по умолчанию использует Node Select',
    globalGroupNodeSelectTip: 'Только для Clash. Явно определяет группу прокси GLOBAL, по умолчанию указывает на Node Select вместо DIRECT, чтобы режим global сразу использовал прокси',
    skipTlsVerify: 'Пропустить проверку TLS',
    skipTlsVerifyTip: 'Только для Node.js/Docker. Позволяет получать подписки без действительного сертификата TLS. Не поддерживается в Cloudflare Workers',
